...
const Product3DViewer = dynamic(() => import("@/components/3d/Product3DViewer"), {
  ssr: false,
  loading: () => (
    <div className="aspect-[4/3] lg:aspect-[16/10] bg-gradient-to-b from-slate-50 to-slate-100 rounded-2xl flex items-center justify-center">
      <div className="text-center">
        <div className="w-16 h-16 border-4 border-blue-200 border-t-blue-600 rounded-full animate-spin mx-auto" />
        <p className="mt-4 text-sm text-slate-500 font-medium">Loading 3D Experience...</p>
      </div>
    </div>
  ),
});
    ...
    <div className="pt-28 pb-20 bg-white">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="mt-12 grid md:grid-cols-2 gap-8">
          {/* Hotspot legend */}
          <div>
            <h3 className="text-lg font-bold text-slate-900 mb-4">Interactive Hotspots</h3>
            <div className="space-y-3">
              {product.hotspots?.map((hotspot, i) => (
                <div key={hotspot.id} className="flex gap-3 p-3 bg-slate-50 rounded-xl">
                  <div className="w-8 h-8 bg-blue-100 text-blue-600 rounded-lg flex items-center justify-center text-sm font-bold shrink-0">
                    {i + 1}
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-slate-800">{hotspot.label}</p>
